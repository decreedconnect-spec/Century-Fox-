...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_sticker("CAACAgUAAxkBAA...")  # sticker file_id
    await update.message.reply_text(
        "Welcome to Century-Fox Auto Post Bot.\n\n"
        "Use /post to schedule a post.\n"
        "Use /voice_on to send your messages as voice notes.\n"
        "Use /voice_off to return to text.\n"
        "Use /queue to view scheduled posts.\n\n"
        "Post format:\n"
        "/post title | message | link | minutes_from_now | video_url(optional)\n\n"
        "Example:\n"
        "/post New Movie Drop | Trailer out now | https://example.com | 5 | https://i.imgur.com/trailer.mp4"
    )

async def voice_on(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user_voice_mode[update.message.from_user.id] = True
    await update.message.reply_text("Voice mode is ON. Your messages will be sent as voice notes.")

async def voice_off(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user_voice_mode[update.message.from_user.id] = False
    await update.message.reply_text("Voice mode is OFF. Your messages will be sent as text.")
...
async def post(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """
    /post title | message | link | minutes | video_url(optional)
    """
    ...
    try:
        ...
        title, message, link, minutes = parts[0], parts[1], parts[2], parts[3]
        video_url = parts[4] if len(parts) == 5 else None
...
async def auto_post_loop(context: ContextTypes.DEFAULT_TYPE):
    ...
    for post_item in post_queue[:]:
        ...
        if now >= post_time:
            ...
            if post_item.get("video_url"):
                await context.bot.send_video(
                    chat_id=CHANNEL_ID,
                    video=post_item["video_url"],
                    caption=caption,
                    parse_mode="HTML",
